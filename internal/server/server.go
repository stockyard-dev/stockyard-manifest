...
func New(db *store.DB) *Server {
	...
	s.mux.HandleFunc("GET /api/dependencys", s.list)
	s.mux.HandleFunc("POST /api/dependencys", s.create)
	s.mux.HandleFunc("GET /api/dependencys/{id}", s.get)
	s.mux.HandleFunc("DELETE /api/dependencys/{id}", s.del)
...
func (s *Server) health(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"status": "ok", "service": "stockyard-manifest", "time": time.Now().UTC().Format(time.RFC3339)})
}

func (s *Server) stats(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"dependencys": s.db.Count()})
}

func (s *Server) list(w http.ResponseWriter, r *http.Request) {
	items := s.db.List()
	if items == nil { items = []store.Dependency{} }
	wj(w, map[string]any{"dependencys": items, "count": len(items)})
}

func (s *Server) create(w http.ResponseWriter, r *http.Request) {
	var e store.Dependency
